...
// SIGNED potentials, spelled `Potential` (<= 0, MPa). psi_soil_ arrives from the
...
// which is std::vector<Potential>. The vulnerability splines take a magnitude, so
// those sites ask for one by name: root_vuln_from_psi.eval(P_src_min.magnitude()
...
public:
  ...
  // trait, quoted as a magnitude, so it is a Suction -- which is what stops it
  // being compared against a signed potential (plant #584; see potential.hpp).
  Suction root_psi_crit{5.870283};
  ...
  // becoming std::vector<Suction>: plant's RcppR6 binds this by name as
  ...
  std::vector<Potential> psi_soil_inverted_;
  // Per-layer cache of root_vuln_integral_from_psi.eval(psi_soil_inverted_[i].magnitude().value).
  ...
  Potential begin_solve() {
    ...
    Potential wettest_soil_layer{-std::numeric_limits<double>::infinity()};
    for (int i = 0; i < max_soil_layer; ++i) {
      // The one flip. psi_soil_ is supplied as a magnitude, so say so and ask for
      // the signed form by name.
      const Potential psi_inverted = Suction{psi_soil_[i]}.signed_potential();
      psi_soil_inverted_[i] = psi_inverted;
      root_vuln_integral_soil_[i] =
          root_vuln_integral_from_psi.eval(psi_inverted.magnitude().value);
  ...
  double duptake_dpsi(Potential P_x_r,
    ...
    for (int i = 0; i < max_soil_layer; i++) {
      ...
      const Potential P_src_min = std::min(psi_soil[i], P_x_r);
      const Potential P_src_max = std::max(psi_soil[i], P_x_r);
      ...
      const Potential hi_neg = std::min(P_src_max, zero);
      const Potential lo_pos = std::max(P_src_min, zero);
      double integral = 0.0;
      if (hi_neg > P_src_min) {
        integral += root_vuln_integral_from_psi.eval(P_src_min.magnitude().value) -
                    root_vuln_integral_from_psi.eval(hi_neg.magnitude().value);
      }
      if (P_src_max > lo_pos) {
...
private:
  ...
  void uptake_impl(Potential P_x_r, const std::vector<Potential>& psi_soil,
    ...
    // begin_solve).
    const double neg_P_x_r = P_x_r.magnitude().value;
    ...
    Potential P_src_min = std::min(psi_soil[i], P_x_r);

    // Find the least negative soil potential out of the given soil layer and the root collar
    Potential P_src_max = std::max(psi_soil[i], P_x_r);
    ...
    if(std::abs(P_x_r - psi_soil[i]) < 1e-8){
      ...
      double f_ri = root_vuln_from_psi.eval(P_src_min.magnitude().value);
      ...
      Potential hi_neg = std::min(P_src_max, zero); // boundary of the psi<=0 part
      Potential lo_pos = std::max(P_src_min, zero); // boundary of the psi>0 part
      ...
      const double neg_psi_soil_i = psi_soil[i].magnitude().value;
      ...
      if (hi_neg > P_src_min) {
        ...
        integral += G_integral(P_src_min.magnitude().value) -
                    G_integral(hi_neg.magnitude().value);
      }
      if (P_src_max > lo_pos) {
