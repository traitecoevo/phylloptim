...
| — *file it* | — | **`bench_gradient` segfaults**, and `make bench` is a pre-push gate | Pre-existing: confirmed by building the same binary from a clean `git archive` of the pre-rename commit, exit 139 with **no output at all**, so it dies during initialisation. Its `Traits{double v[11]}` and `kNames[11]` are stale against a trait vector that has been 14, then 12, now 13, and its header still says "thirteen of the fourteen traits". ⚠️ Until it is fixed, `make -C tests/cpp && make -C tests/cpp bench` cannot pass, so the guide's own pre-push gate is unmeetable |
