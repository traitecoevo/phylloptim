...
| ⚠️ **BLOCKER** | 7a | The four new curves are OFF-PATH, so none is calibratable | `JS22`/`CMax`/`SOX`/`JW26` exist only on the single-layer `optimise_psi_stem_*` solvers. `find_root_collar_psi()` — the production solve, and the one BOTH gradient paths call — is TF24-only. Measured on the single-potential path at `psi_soil` 1.5: the gradient differentiates the TF24 collar optimum at **2.09 MPa** while the JS22 optimum is at **3.15 MPa**, and the `JS22_gamma`/`CMax_a` columns are identically zero. ⚠️ **That zero is NOT the benign structural zero** documented at 7a-ii: same number, opposite meaning — there it means "this parameter does not enter this curve", here it means "the gradient is for a different model than the one you are fitting". This is item 7a's original headline ("dispatch on the PRODUCTION path") and it is what `leaf_calibration_test` is actually waiting on |
