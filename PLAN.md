...
| ✅ landed | 7a-i | Model-scoped parameters take a model-name prefix | **Stage 1 of CMax, and a pure rename.** ⚠️ A plant API break (hazard 7), so land the two together. All three golden files must come out **bit-identical** — that is the entire check, and it is only available if the rename lands alone |
...
| ✅ resolved | 7a | Every model is calibratable through one access point | All eight routes -- `collar` plus the seven `CostCurve` members -- differentiate through `leaf_gradient(model = )` and `leaf_gradient_batch(model = )`, the batch reproducing the single route BIT-FOR-BIT. What unlocked it was naming the BENEFIT LINK: every model is `h(A(psi)) - C(psi)`, so the derivative is one expression, `h'(A)*dA/dpsi - dC/dpsi`, and the product objectives (`log`) and ProfitMax (`A/|A|max`) stop being special cases. ⚠️ ProfitMax's two methods answer different questions -- `"ift"` is the partial at fixed `|A|max`, `"fd"` the total, 32% apart -- so `auto` differences the solve for that curve alone. See `notes/unify-stomatal-optimisers.md` |
...
**And this is now the unification the item asks for, by a different route than it proposed.** 7a's design was that λ should be the pluggable thing. What landed is the **benefit link plus the cost curve**, which is strictly more general: λ is a *consequence* of that pair rather than the primitive, `λ = (dC/dψ)/(dE/dψ)` for whichever pair is in force. `dlambda_dpsi` was never needed — the collar solve's first-order condition is served by `dprofit_dpsi_stem`, which the link makes uniform across all seven curves. Both the off-path optimisers and the gradient are unified, and one access point covers every model.
...
**It is nested in 7a-ii**: `dC/dψ = 2γ(ψ − ψ_soil)` is `CMax_a·ψ + CMax_b` with `CMax_a = 2γ` and `CMax_b = −CMax_a·ψ_soil`.

⚠️ **But they share a family and NOT an implementation, which is a correction to what this item first said.** The plan was one `linear_marginal_cost(psi, a, b)` kernel for both. Measured against it: the general antiderivative `½a(ψ² − ψ_up²) + b(ψ − ψ_up)` disagrees with its factored form `(ψ − ψ_up)[½a(ψ + ψ_up) + b]` by 1.6e-08 at a 1e-08 drop and **1.2e-04** at 1e-12 — a real difference by this guide's own bands, and a gradient's relative 1e-06 step reaches that regime near no flow. CMax must therefore use the factored form; and routing JS22 through it computes `γ(ψ + ψ_up) − 2γψ_up`, reintroducing exactly the cancellation that `γ·Δψ·Δψ` does not have. Two implementations, one documented family.
...
- **No wet-pinned optima, wherever there is a feasible bracket at all.** `dC/dψ → 0` as ψ → ψ_soil while `dA/dψ > 0` there, so `dprofit/dψ` is strictly positive at the wet bound and the optimiser cannot return it. Measured at γ = 20, PPFD 200, VPD 4 — chosen to make closure attractive — ψ* still clears ψ_soil by 1.8e-02 at ψ_soil 5.5 and 1.1e-02 at 5.84. Wet-pinning is the dominant pinned class for TF24 (24 → 80 rows as the leaf warms), so a fit on this curve should route to `"ift"` more often — more exact gradients, fewer finite-difference fallbacks.

  ⚠️ **Not "can never be wet-pinned", which is what this said first and what the same sweep falsified.** Above `psi_crit` (5.870 at the defaults) `psi*` comes back exactly `psi_soil` — but that is the no-flow branch, where there is no feasible interval to be interior in, and not a pinned optimum. The distinction matters to anything reading `psi* == psi_soil` as a decision.
